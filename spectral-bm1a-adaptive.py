...
def start_report():
    ...
    r_head = "time,sweeps,residual,error\n"
...
def main():
    ...
    for check in CheckpointStepper(start=t,
        ...
        stepper = PIDStepper(start=check.begin,
                             stop=check.end,
                             size=dt,
                             limiting=False,
        ...
        for step in stepper:
            ...
            scaled_error = 0.1 * sweeps * np.linalg.norm(
                evolve_ch.c - evolve_ch.c_old,
                ord=1)

            residues.append([t, sweeps, residual, scaled_error])
